 ...
 * LogDetailsScreen 
 * 
 * This screen displays detailed nutritional information for a specific logged food item.
 * It calculates nutrient totals based on the logged quantity and shows progress percentages relative to the 
 * user's daily goals. The screen formats and presents a variety of nutrients, including macros, vitamins, 
 * and minerals, along with the log time and meal info. Navigation buttons allow quick access back to the home screen, 
 * the food logging screen, and the user's full food log.
 */

const LogDetailsScreen = ({ route, navigation }) => {
  const { log, user, foodItem, foodType } = route.params;
  const calories = foodItem.calories * log.quantity;
  const protein = foodItem.protein * log.quantity;
  const carbs = foodItem.carbs * log.quantity;
  const fat = foodItem.fat * log.quantity;
  const totalSugars = foodItem.totalSugars * log.quantity;
  const addedSugars = foodItem.addedSugars * log.quantity;
  const cholesterol = foodItem.cholesterol * log.quantity;
  const transFat = foodItem.transFat * log.quantity;
  const saturatedFat = foodItem.saturatedFat * log.quantity;
  const polyunsaturatedFat = foodItem.polyunsaturatedFat * log.quantity;
  const monounsaturatedFat = foodItem.monounsaturatedFat * log.quantity;
  const fiber = foodItem.fiber * log.quantity;
  const calcium = foodItem.calcium * log.quantity;
  const iron = foodItem.iron * log.quantity;
  const sodium = foodItem.sodium * log.quantity;
  const potassium = foodItem.potassium * log.quantity;
  const vitaminA = foodItem.vitaminA * log.quantity;
  const vitaminC = foodItem.vitaminC * log.quantity;
  const vitaminD = foodItem.vitaminD * log.quantity;
  ...
  // Function to convert time to AM/PM format for display
    ...
    <View style={styles.container}>
      ...
      <View style={styles.innerContainer}>
        ...
        <View style={styles.titleContainer}>
          <Text numberOfLines={2} ellipsizeMode="tail" style={styles.title}>{log.foodItemName}</Text>
        </View>
        <Text style={styles.nutrients}>Log Time: {formatTimeToAMPM(log.logTime)}</Text>
        <Text style={styles.nutrients}>{foodType}</Text>
        <Text style={styles.nutrients}>Meal: {log.selectedMeal}</Text>
        <Text style={styles.servingText}>{log.selectedUnit ? `${log.unitQuantity} ${log.selectedUnit}` : log.unitQuantity.toFixed()}</Text>
