...
import { View, Text, TextInput, Button, StyleSheet, Alert, TouchableOpacity, Modal, 
...
/**
 * LoggedFoodDetailsScreen
 *  
 * Allows users to view and update the quantity, serving unit, and associated meal for a food log entry.
 * Dynamically calculates and displays detailed nutritional information based on user inputs.
 * Supports editing, saving changes to the backend, and deleting the logged food entry.
 */

const { width, height } = Dimensions.get('window'); 

const LoggedFoodDetailsScreen = ({ route }) => {
  const navigation = useNavigation();
  
  const { log: foodLog, user, foodItem } = route.params;
  const [quantity, setQuantity] = useState(foodLog ? foodLog.unitQuantity : 0);
  const [selectedMeal, setSelectedMeal] = useState(foodLog ? foodLog.selectedMeal : '');
  ...
  const styles = dynamicStyles(colorScheme, isFirstSection, insets, foodItem);
  
  // Function to determine appropriate serving unit labels when quantity changes
  useEffect(() => {
    ...
    if (!isNaN(qty)) {
      ...
      // Update 'serving' to 'servings' if quantity is greater than 1 
  ...
  useEffect(() => {
    if (foodItem) {
      ...
      // Calculate nutrient values based on the serving size unit selected by user
  ...
  // Function to submit updated food log entry
  const handleLogFood = () => {
    ...
    // Ensure serving units are finished loading
    ...
    // Determine log quantity based on user selection
    switch (selectedUnit) {
      case 'oz.':
        logQuantity *= 28.3495;
        break;
    ...
    // Send request to update food log entry
  ...
  const formatNumber = (value) => {
    const roundedValue = Math.round(value * 10) / 10; // Round the number to one decimal place
  ...
  // Function to determine serving text to display in description
  ...
  // Function to delete food log entry
...
const dynamicStyles = (colorScheme, isFirstSection, insets, foodItem) => StyleSheet.create({
  container: {
    flex: 1,
  ...
  deleteButton: {
    paddingTop: foodItem ? foodItem.name.length < 38 ? 45 : 25 : 45,
    position:'absolute',
    alignSelf: 'center',
  },
  deleteText: {
...
export default LoggedFoodDetailsScreen;
