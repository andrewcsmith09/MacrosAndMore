 ...
 * MealDetailsScreen 
 * 
 * This screen displays the nutritional totals for an entire meal selected on the 
 * My Food Log screen and compares each nutrient to the user's daily goals. 
 * It shows values like calories, macronutrients, vitamins, and minerals, 
 * along with their respective percentages of daily intake.
 */

const MealDetailsScreen = ({ route, navigation }) => {
  const { selectedMeal, mealTotals, user } = route.params;
  const calories = mealTotals?.calories
  const protein = mealTotals?.protein;
  const carbs = mealTotals?.carbs;
  const fat = mealTotals?.fat;
  const totalSugars = mealTotals?.totalSugars;
  const addedSugars = mealTotals?.addedSugars;
  const cholesterol = mealTotals?.cholesterol;
  const transFat = mealTotals?.transFat;
  const saturatedFat = mealTotals?.saturatedFat;
  const polyunsaturatedFat = mealTotals?.polyunsaturatedFat;
  const monounsaturatedFat = mealTotals?.monounsaturatedFat;
  const fiber = mealTotals?.fiber;
  const calcium = mealTotals?.calcium;
  const iron = mealTotals?.iron;
  const sodium = mealTotals?.sodium;
  const potassium = mealTotals?.potassium;
  const vitaminA = mealTotals?.vitaminA;
  const vitaminC = mealTotals?.vitaminC;
  const vitaminD = mealTotals?.vitaminD;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.innerContainer}>
        ...
        <View style={styles.titleContainer}>
          <Text style={styles.title}>{selectedMeal} Totals</Text>
        </View>
...
const dynamicStyles = (colorScheme, width, height, insets) => StyleSheet.create({
  ...
  detailValues: {
    fontSize: insets.bottom === 0 ? height * .018 : height * 0.0165,
